...
if __name__ == '__main__':
    ...
    if settings.DIRECTED:
        G = nx.DiGraph()
    if (settings.STATIC_REMBEDDING and settings.YEAR_CURRENT%3==0):
        edgestoembed=pd.read_csv(f"{settings.DIRECTORY}{EDGES_DIR_CUMULATIVE}/edges{JOIN}{splitjoin}.csv")
        G= nx.from_pandas_edgelist(edgestoembed,source='Source',target='Target',create_using=G)
        ...
        G_model['id'] = G_model['id'].astype('int')

        #G_model=G_model.set_index('id')
        targets= pd.read_csv(f'{settings.DIRECTORY}nodes/nodescomplete.csv')
        targets= targets[targets['Year']<=JOIN]
        
        targets= targets.drop(['Label'], axis=1)
        print(targets.dtypes)
        print(G_model.dtypes)
        
        df= pd.merge(G_model,targets,how='left', on='id')
        df=df.set_index('id')
        print(df)
        dfstart=df[df['Year']<=START]
        dfend=df[df['Year']==JOIN]
        with open(f'{settings.DIRECTORY}embeddings/{settings.BASE_ALGORITHM}_{settings.NAME_DATA}{START}{splitstart}model.csv','w+', newline='') as f:
            dfstart= dfstart.drop('Year', axis=1)
            dfstart.to_csv(f,sep=' ', header=False, index=True)
        with open(f'{settings.DIRECTORY}{settings.EMBEDDING_DIR}{settings.INCREMENTAL_MODEL}_{settings.BASE_ALGORITHM}_{settings.YEAR_START+settings.YEAR_CURRENT}{splitjoin}.csv','w+', newline='') as f:
            dfend= dfend.drop('Year', axis=1)
            dfend.to_csv(f,sep=' ', header=False, index=True)
    else:
        ...
        if EMBED_G and ((settings.YEAR_START==START and not (settings.SPLIT_NODES and settings.HALF_YEAR==1))):
            ...
            if (settings.YEAR_CURRENT==1 and not (settings.SPLIT_NODES and settings.HALF_YEAR==1)):
                ...
                for i in range (1,settings.YEAR_CURRENT):
                    ...
                    if os.stat(f'./{settings.DIRECTORY}{settings.EMBEDDING_DIR}{settings.NAME_DATA}_incremental_{settings.BASE_ALGORITHM}_{settings.YEAR_START+i}.csv').st_size != 0:
                ...
                if settings.SPLIT_NODES and settings.HALF_YEAR==1:
                    if os.stat(f'./{settings.DIRECTORY}{settings.EMBEDDING_DIR}{settings.NAME_DATA}_incremental_{settings.BASE_ALGORITHM}_{settings.YEAR_START+settings.YEAR_CURRENT}_higher.csv').st_size != 0:
        ...
        if settings.SPLIT_NODES and settings.HALF_YEAR==1:
            if os.stat(f'{settings.DIRECTORY}{settings.EMBEDDING_DIR}{settings.INCREMENTAL_MODEL}_{settings.BASE_ALGORITHM}_{settings.YEAR_START+settings.YEAR_CURRENT}_higher.csv').st_size > 0:
            ...
            else: embhigher=pd.DataFrame()
            if os.stat(f'{settings.DIRECTORY}{settings.EMBEDDING_DIR}{settings.INCREMENTAL_MODEL}_{settings.BASE_ALGORITHM}_{settings.YEAR_START+settings.YEAR_CURRENT}_lower.csv').st_size > 0:
