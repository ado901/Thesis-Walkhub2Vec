...
def staticscore(STATIC_ALGORITHM):
    ...
    targets= targets[targets['Year']<=settings.YEAR_START+settings.YEAR_CURRENT]
    
    df= pd.merge(df,targets,how='left', on='id')
    df.rename(columns = {'Year':129}, inplace = True)
    df=shuffle(df)
    dfstart=df[df[129]<=settings.YEAR_START+settings.YEAR_CURRENT-1]
    dfend=df[df[129]==settings.YEAR_START+settings.YEAR_CURRENT]
