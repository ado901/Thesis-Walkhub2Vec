...
def get_graph_T(graph_nx, min_time=-np.inf, max_time=np.inf, return_df=False, time='time'):
    '''
...
def get_graph_times(graph_nx, time='time'):
    '''
...
def get_pivot_time(graph_nx, wanted_ratio=0.2, min_ratio=0.1,timecol='time'):
    '''
    ...
    for time in times[int(len(times) / 3):]:
        train_graph_nx = multigraph2graph(get_graph_T(graph_nx, max_time=time,time=timecol))
        num_edges_train = len(train_graph_nx.edges())

        test_graph_nx = get_graph_T(graph_nx, min_time=time,time=timecol)
