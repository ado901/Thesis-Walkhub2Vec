...
class tNodeEmbed(TaskModel):
    def __init__(self, graph_nx, task, dump_folder, test_size=0, align=True,time='time', **n2vargs):
        '''
        tNodeEmbed init
        Args:
        ...
        times = get_graph_times(self.graph_nx, time=time)
        self.timecol=time
    ...
    def calculate_pivot_time(self):
        ...
        pivot_time = get_pivot_time(self.graph_nx, self.test_size,timecol=self.timecol)
    ...
    def initialize(self):
        ...
        if os.path.exists(init_path):
            ...
            graph_nx = tNodeEmbed._initialize_embeddings(self.graph_nx, self.train_time_steps,timecol=self.timecol, **self.n2vargs)
    ...
    @staticmethod
    def _initialize_embeddings(graph_nx, times=None,timecol='time', **n2vargs):
        '''
        ...
        for time in tqdm(times, desc='Time Steps', unit='time_step'):
            cur_graph_nx = get_graph_T(graph_nx, max_time=time,time=timecol)
            cur_graph_nx = multigraph2graph(cur_graph_nx)

            node2vec = Node2Vec(graph=cur_graph_nx, quiet=True, **n2vargs)
            ntv_model = node2vec.fit().wv
            ntv = {node: ntv_model[str(node)] for node in list(cur_graph_nx.nodes())}
