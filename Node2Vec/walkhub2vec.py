   ...
   # print(H.nodes())
    ...
    if EMBED_G:
        if settings.BASE_ALGORITHM == "node2vec":
            ...
            G_model= Deepwalk(f"{settings.DIRECTORY}{settings.NAME_DATA}{settings.YEAR_START}_G_edges.csv",settings.DIRECTED,settings.EMBEDDING_DIR,f"{settings.NAME_DATA}{settings.YEAR_START}_{settings.BASE_ALGORITHM}_G",EMBEDDING_WORKERS,settings.WINDOWS_SIZE,settings.DIMENSION,settings.NUM_WALKS,settings.LENGTH_WALKS,separator=',')
            G_model.wv.save_word2vec_format(f'./{settings.DIRECTORY}{settings.EMBEDDING_DIR}{settings.BASE_ALGORITHM}_{settings.NAME_DATA}{settings.YEAR_START}modelpreload.csv')
            G_model=KeyedVectors.load_word2vec_format(f'./{settings.DIRECTORY}{settings.EMBEDDING_DIR}{settings.BASE_ALGORITHM}_{settings.NAME_DATA}{settings.YEAR_START}modelpreload.csv')
            os.remove(f'{settings.DIRECTORY}{settings.EMBEDDING_DIR}/bin/{settings.NAME_DATA}{settings.YEAR_START}_{settings.BASE_ALGORITHM}_G.bin') #risparmiare spazio su disco :)
            os.remove(f'{settings.DIRECTORY}{settings.EMBEDDING_DIR}/emb/{settings.NAME_DATA}{settings.YEAR_START}_{settings.BASE_ALGORITHM}_G.emb')
    ...
    nodes_list=[]
    edges_lists=[]
    G_model.save_word2vec_format(f'./{settings.DIRECTORY}{settings.BASE_ALGORITHM}_{settings.NAME_DATA}{settings.YEAR_START}model.csv')
    with open(f'./{settings.DIRECTORY}{settings.BASE_ALGORITHM}_{settings.NAME_DATA}{settings.YEAR_START}model.csv', 'r') as fin:
            data = fin.read().splitlines(True)
    with open(f'./{settings.DIRECTORY}{settings.BASE_ALGORITHM}_{settings.NAME_DATA}{settings.YEAR_START}model.csv', 'w') as fout:
